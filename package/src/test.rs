...
fn test_tools_pex_reproducibility(
    ...
    let pants_tools_pex_pex_info = execute(
        Command::new(&pants_tools_pex_path)
            .args(["info"])
            .env("PEX_TOOLS", "1")
            .stderr(Stdio::piped()),
    )
    .unwrap().stdout;
let our_tools_pex_info = execute(
    Command::new(&tools_pex_path)
        .args(["info"])
        .env("PEX_TOOLS", "1")
        .stderr(Stdio::piped()),
)
.unwrap().stdout;
    ...
    if !tools_pex_mismatch_warn {
        assert_eq!(pants_tools_pex_pex_info, our_tools_pex_info);
    } else if pants_tools_pex_pex_info != our_tools_pex_info {
        log!(
            Color::Yellow,
            ...
            pants = pants_tools_pex_pex_info,
            our_tools_path = tools_pex_path.display(),
            ours = our_tools_pex_info,
            eol = EOL,
        );
    }
}
